...
public class TarMethod extends MethodProvider{

	
	
	public TarMethod(MethodContext ctx) {
		super(ctx);
	}
Timer timer = new Timer(0);
public GroundItem getGItem(int i) {
	for(GroundItem obj : ctx.groundItems.select().id(i).nearest().first()){
		return obj;
	}
	return null;
}
	...
	public void clickOnMap(Tile t) {
		...
		public boolean objIsByTile(Tile tile, int object, int dist) {
		 ...
		 public boolean objIsNotNull(String name) {
			...
			public void interactG(final int id, final String string, final String o) {
				for(GroundItem y: ctx.groundItems.select().id(id).nearest().first()){
					TarBody.state = o;
							if (y.isOnScreen()) {
								
								if(y.interact(string)){
									ctx.game.sleep(Random.nextInt(900, 1200));
									TarBody.waitGrabTar = new Timer(Random.nextInt(900, 1200));
								}
	...
	public void teleportTo(int loc, String teleName) {
		while(ctx.bank.isOpen())
			ctx.bank.close();
		TarBody.state = "Teleporting to: " + teleName;
		if(!timer.isRunning()){
