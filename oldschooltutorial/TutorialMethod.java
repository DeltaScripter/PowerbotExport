...
public class TutorialMethod extends ClientAccessor{

	
	
	public TutorialMethod(ClientContext ctx) {
		super(ctx);
	}
		 ...
		 public boolean objIsNotNull(String name) {
			...
			public void interactO(final int id, final String string, final String o) {
				for(GameObject y: ctx.objects.select().id(id).nearest().first()){
					TutorialBody.state = o;
							if (y.inViewport()) {
								if(!ctx.players.local().inMotion())
								y.interact(string);
							} else ctx.camera.turnTo(y.tile());
			...
			public boolean inventoryContains(int ID) {
			  System.out.println("Has it? "+!ctx.inventory.select().id(ID).isEmpty());
				return !ctx.inventory.select().id(ID).isEmpty();
			}
	public void interactInventory(final int id, final String string, final String o) {
		int INVENTORYWIDGET = 149;
		
		if(!ctx.widgets.component(INVENTORYWIDGET, 0).visible()){
	    ctx.mouse.click(642,185,true);//click on inventory button to open it up
		}else
		for(Item g: ctx.inventory.items()){
			if(g.id()==id){
				g.interact(string);
				System.out.println("Interacted with item in inventory: " + string);
			}
		}
