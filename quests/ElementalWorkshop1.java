...
public class ElementalWorkshop1 extends Node{
		...
		if((ctx.settings.get(2675)>>6&0x1)==1){//if bellows is fixed
			if((ctx.settings.get(2675)>>7&0x1)==1){//If furnace is fixed
				if(Method.inventoryContains(2893)){
					...
					if(new Tile(2718,9890,0).distanceTo(local.getLocation())<4){
						if(ctx.widgets.get(1370).isValid()){
							if(ctx.widgets.get(1370).getComponent(56).getText().contains("Elemental shield")){
								ctx.widgets.get(1370).getComponent(38).click(true);
							}else ctx.widgets.get(1371).getComponent(44).getChild(2).click(true);
						}else if(!local.isInMotion())
				...
				if(Method.inventoryContains(2892)){
					...
					if(new Tile(2734,9888,0).distanceTo(local.getLocation())<4){
				...
				if(Method.objIsNotNull(2892)){//ore
					if(Method.getObject(2892).getLocation().distanceTo(local.getLocation())<4){
						Method.interactG(2892, "Take", "Elemental ore");
					}else Method.clickOnMap(Method.getObject(2892).getLocation());
				}else
				if(new Tile(2704,9896,0).distanceTo(local.getLocation())<7){
	...
	if(Method.objIsNotNull(18585)){
		...
		if((ctx.settings.get(2675)>>4&0x1)==1){//right lever(water controls)
			...
			if(new Tile(2713,9907,0).distanceTo(local.getLocation())<3){//the water controls to the right of the room
				Method.interactO(3404, "Turn", "Water controls");
			}else Method.clickOnMap(new Tile(2713,9907,0));
			}
		}else{
			DeltaQuester.progress = 2;
		if(new Tile(2726,9907,0).distanceTo(local.getLocation())<3){//the water controls to the right of the room
			Method.interactO(3403, "Turn", "Water controls");
		}else Method.clickOnMap( new Tile(2726,9907,0));
		}
	}else
		if(new Tile(2709,3493,0).distanceTo(local.getLocation())<4){
			if(EntranceDoor.contains(local.getLocation())){
				Method.interactO(25795, "Climb", "Stairs");
			}else Method.interactO(26114, "Open", "Wall");
		}else if(new Tile(2709,3493,0).distanceTo(local.getLocation())<30){
			if (Method.objIsNotNull(25819) && StartDoor.contains(Method.getObject(25819).getLocation())){
	...
	private void cs0() {//Start the quest
		...
		if(new Tile(2712,3483,0).distanceTo(local.getLocation())<4||new Tile(2715,3481,0).distanceTo(local.getLocation())<3){
			if(Method.objIsNotNull(25819) && !StartDoor.contains(Method.getObject(25819).getLocation()) || !Method.objIsNotNull(25819)){
				if(new Tile(2715,3481,0).distanceTo(local.getLocation())<2){
					if(Method.inventoryContains(2886)){
		...
		}else if(TeleportLode.CATHERBY.getTile().distanceTo(local.getLocation())<10 || TeleportLode.SEERS.getTile().distanceTo(local.getLocation())<10){
			Vars.DYNAMICV = true;
