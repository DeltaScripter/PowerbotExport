...
public class DeathPlateau extends Node{
	...
	private void cs6() {//speak to nd destroy the troll
		...
		if(Method.getInteractingNPC()!=null && ctx.players.local().inCombat()){//being attacked
			Vars.DYNAMICV = false;
	...
	private void cs5(){
		...
		if(!failsafe())
		if(new Tile(3432,4241,2).matrix(ctx).reachable()){//last obs area
			if(new Tile(3435,4240,2).distanceTo(local.tile())<7){//last obs
				Method.interactO(67569, "Exit", "Cliffside hole");
			}else Method.clickOnMap(new Tile(3435,4240,2));//last obs
		}else
		if(new Tile(3420,4239,1).matrix(ctx).reachable()){//fith obs area
			if(new Tile(3420,4239,1).distanceTo(local.tile())<7){//fith obs
				Method.interactO(17030, "Climb", "Cliffside");
			}else Method.clickOnMap(new Tile(3420,4239,1));//fith obs
		}else		
		if(new Tile(3414,4260,1).matrix(ctx).reachable()){//fith obs area
			if(new Tile(3417,4254,1).distanceTo(local.tile())<5){//fith obs
				if(local.animation()==-1)
				Method.interactO(67676, "Squeeze", "Gap");
			}else Method.clickOnMap(new Tile(3417,4254,1));//fith obs
		}else	
		if(new Tile(3428,4261,1).matrix(ctx).reachable()){//fourth obs area
			if(new Tile(3423,4260,1).distanceTo(local.tile())<7){//fourth obs
				Method.interactO(67679, "Jump", "Stepping stones");
			}else Method.clickOnMap(new Tile(3423,4260,1));//fourth obs
		}else
		if(new Tile(3434,4274,1).matrix(ctx).reachable()){//third obs area
			if(new Tile(3435,4261,1).distanceTo(local.tile())<7){//third obs
				Method.interactO(67752, "Swing-on", "Rope swing");
			}else Method.clickOnMap(new Tile(3435,4261,1));//third obs
		}else
		if(new Tile(3428,4279,1).matrix(ctx).reachable()){//second obs area
			if(new Tile(3434,4278,1).distanceTo(local.tile())<7){//second obs
				if(local.animation()==-1)
				Method.interactO(67678, "Jump", "Crevice");
			}else Method.clickOnMap(new Tile(3434,4278,1));//second obs
		}else
		if(new Tile(3419,4280,1).distanceTo(local.tile())<7){//first obs
			if(local.animation()==-1)
			Method.interactO(67667, "Squeeze", "Gap");
		}else if(new Tile(3406,4278,1).matrix(ctx).reachable()){
			if(!local.inMotion())
			Method.clickOnMap(new Tile(3419,4280,1));//get to the first obs
		}else
		if(new Tile(3405,4283,2).distanceTo(local.tile())<7){//Cave initial entrance
			Method.interactO(67674, "Jump-down", "Cliff");
		}else
		if(new Tile(2267,4759,0).distanceTo(local.tile())<15){//Inside dwarf room(cave)
			if(new Tile(2267,4759,0).distanceTo(local.tile())<5){//area by cave entrance
	...
	private boolean failsafe() {
		if(new Tile(3432,4258,0).matrix(ctx).reachable()){
			Method.interactO(7103,"Climb", "Rock ladder");
			return true;
		}else
		if(new Tile(3432,4276,0).matrix(ctx).reachable()){
			Method.interactO(7103,"Climb", "Rock ladder");
			return true;
		}else if(new Tile(3419,4259,0).matrix(ctx).reachable()){
	...
	private void cs4() {//Reads a scroll given to us
		...
		Method.skipPics();
		if(!Method.isChatting("Dwarf"))
		if(ctx.widgets.component(1242,1).valid()){
	...
	private void cs2() {//Walks to and speaks with the other dwarf to get the plans
		...
		if(Method.objIsNotNull(67568)){//Below exits the cave you're no supposed to be in
			if(Method.getObject(67568).tile().distanceTo(local.tile())<6){
				Vars.DYNAMICV = true;
				Method.interactO(67568, "Exit", "Cave exit");
			}else Method.clickOnMap(Method.getObject(67568).tile());//cave area
		}else
		
		if(new Tile(2824,3555,0).distanceTo(ctx.players.local().tile())<8){//location by other dwarf
			if(Method.objIsNotNull(3725) && DwarfDoor1.contains(Method.getObject(3725).tile())){
				if(DwarfDoor1.contains(Method.getObject(3725).tile())){
						Method.interactO(3725, "Open","Door");
				}else Method.interactO(67638, "Open","Door");
			}else
			if(Method.objIsNotNull(67638) && DwarfDoor2.contains(Method.getObject(67638).tile())){
	...
	private void cs1() {//Enters the dwarf's cave and speaks to him; then mines the wall
		...
		}else if(new Tile(2859,3577,0).distanceTo(ctx.players.local().tile())<6){
			Method.interactO(34395, "Enter","Cave");//enter the cave.
		}else if(Vars.DYNAMICV){
			if(new Tile(2858,3577,0).distanceTo(ctx.players.local().tile())<20){
	...
	private void cs0() {//Speaks to the captain to begin the quest
		...
		if(new Tile(2910,3534,0).distanceTo(ctx.players.local().tile())<6){
